import { GoldpingerData, NetworkNode, NetworkLink, NetworkData } from '../types/goldpinger'
...
export function transformToNetworkData(goldpingerData: GoldpingerData): NetworkData {
  const nodes: NetworkNode[] = []
  const links: NetworkLink[] = []
  const podIPs: string[] = []

  // Process pod nodes from responses
  for (const [podIP, responseData] of Object.entries(goldpingerData.responses)) {
    nodes.push({
      id: podIP,
  ...
  for (const [sourceIP, responseData] of Object.entries(goldpingerData.responses)) {
    if (responseData.response?.podResults) {
      for (const [targetIP, podResult] of Object.entries(responseData.response.podResults)) {
        links.push({
          source: sourceIP,
          target: targetIP,
  ...
  if (goldpingerData.probeResults) {
    for (const [targetHost, podResults] of Object.entries(goldpingerData.probeResults)) {
...
export function transformToSigmaData(networkData: NetworkData): { nodes: SigmaNode[], edges: SigmaEdge[] } {
  ...
  networkData.links.forEach((link, i) => {
    ...
    edges.push({
      ...
      type: 'arrow',
      size: isExternal ? 1 : 2,  // Thinner lines for external connections
...
function getEdgeColor(status: 'healthy' | 'unhealthy' | 'unknown', type: 'pod-to-pod' | 'pod-to-external'): string {
